...
typedef struct GLMmodelNode
{
	GLMmodel* modelData;
	int modelID;
	Vector MaxCoords;	//Maximum calculate vector
	Vector MinCoords;	//Minimum calculated vector
	GLMmodelNode *next;
}GLMmodelNode;
...
{
	GLMmodel* modelData;
	char* name; // as generated or by label
	int modelID;   //model (obj) ID
	int instanceID; //instance ID
	int dID; //dynamic Model ID (itemID in MazeFile)
	Vector pos;		// current position
	Vector rot;		// current rotation
	Vector initPos;		// initial position
	Vector initRot;		// initial rotation
	GLfloat scale,initScale;	// current scale, initial scale
	bool collide;	//enables object collision
	bool kinematic;
	float mass;
	Vector MaxCoords;	//Maximum calculate vector
	Vector MinCoords;	//Minimum calculated vector
	bool dynamic;	//object is moveable/activatable
	bool drawFlag; //mark if object should be drawn still
	MapModel *next;
	int triggerAction;	//Trigger action type stored as int  : 0=No action 1=Move/Scale/Rotate 2=Change Model 3=Destroy Model
	int triggerCriteria;	//0=No action 1=Proximity 2=Time 3=Device 4=Proximity and Time 5=Device(Interact) and Time 6=Proximity and Device(Interact) 7=Proximity Device(Interact) and Time
	int highlightCriteria;	//0=No action 1=Proximity 2=Time 3=Proximity and Time
	int highlightStyle; //1=bounce (Y axis) 2=shake (x axis) 3= rotate, 4=glow Red, 5= glow Green, 6 = glow blue 10=glow
	bool p2InteractRequired;
	double p1ActiveRadius; //highlight radius
	double p2ActiveRadius; //triggered Radius
	btRigidBody* body; //Bullet Rigid Body info
	btTriangleMesh* colMesh; //collMesh
	...
	float BBoxCoords[8][3];
	
	float roughRadius;
	float glowFactor;
	int glowColor;
	Vector endPos,endRot; //Final Position after trigger, relative to current position
	GLfloat endScale,currentPercentage,actionTime;
	bool activated;
	bool moving,destroy,destroyed;
	int targetModelID;
	GLfloat targetPercentage;
	bool switchModel;
	bool highlighted,bouncingUp;
...
GLvoid
glmCalcBounds(MapModel* model);

GLvoid
glmMovePercent(MapModel* model, double movePercent);

GLvoid
glmReducedDraw(GLMmodel* model);
