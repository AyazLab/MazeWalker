...
namespace freetype {
	...
	int make_dlist(FT_Face face, unsigned short ch, GLuint list_base, GLuint * tex_base) {
		...
		glTranslatef(0, fontoffset + bitmap_glyph->top - bitmap.rows, 0);
	...
	int font_data::getPixelLength(wchar_t* wstr)
	{
		return font_data::extend(wstr);
	}
	...
	void print(font_data &ft_font, float x, float y, int allign, wchar_t *fmt, ...)  {
		...
		wchar_t		text[512];								// Holds Our String
		...
		for (int i = 0; i < lineCount; i++)
		{
			...
			glTranslatef(x - offset, y - h*(i+1)-fontoffset, 0);
